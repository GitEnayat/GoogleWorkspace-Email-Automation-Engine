...
export class NodeGoogleSheetsTableRenderer implements TableRenderer {
  ...
  async renderTable(sheetId: string, range: string): Promise<string> {
    ...
    const columnMetadata = (sheet as any)?.basicViews?.columnMetadata || [];
    ...
    for (let i = 0; i < numRows; i++) {
      ...
      for (let j = 0; j < numCols; j++) {
        ...
        const bgColor = bg
          ? `rgb(${Math.round((bg.red || 0) * 255)},${Math.round((bg.green || 0) * 255)},${Math.round((bg.blue || 0) * 255)})`
          : "#ffffff";
        const font = format.textFormat || {};
        const textColor = font.foregroundColor
          ? `rgb(${Math.round((font.foregroundColor.red || 0) * 255)},${Math.round((font.foregroundColor.green || 0) * 255)},${Math.round((font.foregroundColor.blue || 0) * 255)})`
          : "#000000";
        ...
        const styles = [
          ...
          `text-align: ${this.mapAlignment(format.horizontalAlignment ?? undefined)}`,
          `vertical-align: ${this.mapVerticalAlignment(format.verticalAlignment ?? undefined)}`,
