...
export class NodeGoogleEmailProvider implements EmailProvider {
  ...
  async sendEmail(draftId?: string): Promise<void> {
    if (!draftId)
      throw new Error("draftId is required for NodeGoogle sendEmail");
  ...
  getCurrentUserEmail(): string {
    if (this.userEmail !== "me") {
      return this.userEmail;
    }
    throw new Error(
      "getCurrentUserEmail() is async - use getCurrentUserEmailAsync() instead",
    );
  }

  async getCurrentUserEmailAsync(): Promise<string> {
    try {
      const response = await this.gmail.users.getProfile({
        userId: "me",
      });
      return response.data.emailAddress || "default-authorized-user@google.com";
    } catch (error) {
      return "default-authorized-user@google.com";
    }
  }
